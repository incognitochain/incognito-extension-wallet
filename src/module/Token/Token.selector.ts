...
export const isTokenBySymbolSelector = createSelector(
  popularCoinIdsSeletor,
  (coins) => (symbol: string) => !!coins[symbol]
);
...
export const isErc20TokenSelector = createSelector(
  ...
  (selectedPrivacy) =>
    ...
    selectedPrivacy.symbol === COINS.CRYPTO_SYMBOL.ETH
);

export const gettingBalanceSelector = createSelector(
  tokenSelector,
  (token) => token.gettingBalance
);

export const isGettingBalanceTokenByIdSelector = createSelector(
  gettingBalanceSelector,
  (gettingBalance: string[]) => (tokenId: string) =>
    gettingBalance.includes(tokenId)
);
